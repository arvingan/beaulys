...
function step3(&$install_error,&$install_recover){
	...
	if(!($mysqli_link = mysqli_connect($db_host.":".$db_port, $db_user, $db_pwd))) {
		$install_error = '数据库连接失败';return;
	}

	if(mysqli_get_server_info($mysqli_link) > '5.0') {
		mysqli_query($mysqli_link, "CREATE DATABASE IF NOT EXISTS `$db_name` DEFAULT CHARACTER SET ".DBCHARSET);
	} else {
		$install_error = '数据库必须为MySQL5.0版本以上';return;
	}
	if(mysqli_errno($mysqli_link)) {
		$install_error = mysql_error();return ;
	}
	if($_POST['install_recover'] != 'yes' && ($query = mysqli_query($mysqli_link, "SHOW TABLES FROM $db_name"))) {
